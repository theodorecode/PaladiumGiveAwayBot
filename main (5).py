...
async def ajouter_jetons(interaction: discord.Interaction, membre: discord.Member, quantite: int):
    ...
    if uid not in data["joueurs"][gid]:
        data["joueurs"][gid][uid] = {"jetons": 0}
    data["joueurs"][gid][uid]["jetons"] += quantite
    save_loto(data)
    embed = discord.Embed(title="🎟️ Jetons ajoutés", description=f"{membre.mention} a reçu **{quantite} jeton(s)** !\nSolde total : **{data['joueurs'][gid][uid]['jetons']} jeton(s)**", color=0xf5a623)
...
async def retirer_jetons(interaction: discord.Interaction, membre: discord.Member, quantite: int):
    ...
    gid = str(interaction.guild.id)
    uid = str(membre.id)
    if gid not in data["joueurs"] or uid not in data["joueurs"][gid]:
        await interaction.response.send_message("❌ Ce joueur n'a pas de jetons.", ephemeral=True)
        return
    data["joueurs"][gid][uid]["jetons"] = max(0, data["joueurs"][gid][uid]["jetons"] - quantite)
    save_loto(data)
    await interaction.response.send_message(embed=discord.Embed(title="❌ Jetons retirés", description=f"{membre.mention} a perdu **{quantite} jeton(s)**.\nSolde : **{data['joueurs'][gid][uid]['jetons']} jeton(s)**", color=0xe74c3c))

@bot.tree.command(name="jetons", description="🎟️ Voir ton solde de jetons")
...
async def jetons(interaction: discord.Interaction, membre: discord.Member = None):
    ...
    gid = str(interaction.guild.id)
    uid = str(cible.id)
    solde = data.get("joueurs", {}).get(gid, {}).get(uid, {}).get("jetons", 0)
    embed = discord.Embed(title=f"🎟️ Jetons de {cible.display_name}", description=f"Solde : **{solde} jeton(s)**", color=0xf5a623)
    await interaction.response.send_message(embed=embed)
...
# ─── Machine à sous ───────────────────────────
# 🍒🍒🍒 apparaît 3x plus, 🍋🍊 2x → ~1 chance sur 8 de gagner
SLOTS_SYMBOLS = ["🍒", "🍒", "🍒", "🍋", "🍋", "🍊", "🍊", "⭐", "💎", "7️⃣"]
SLOTS_WINS = {
    ("💎", "💎", "💎"): 50,
    ("7️⃣", "7️⃣", "7️⃣"): 30,
...
@bot.tree.command(name="slot", description="🎰 Jouer à la machine à sous (coûte 1 jeton)")
async def slot(interaction: discord.Interaction):
    ...
    solde = data.get("joueurs", {}).get(gid, {}).get(uid, {}).get("jetons", 0)
    if solde < 1:
        await interaction.response.send_message("❌ Tu n'as pas assez de jetons ! (il en faut 1)", ephemeral=True)
        return
    data["joueurs"][gid][uid]["jetons"] -= 1
    result = [random.choice(SLOTS_SYMBOLS) for _ in range(3)]
    gain = SLOTS_WINS.get(tuple(result), 0)
    if gain > 0:
        data["joueurs"][gid][uid]["jetons"] += gain
        titre = "🎰 JACKPOT !" if gain >= 20 else "🎰 Gagné !"
        desc = f"{'  '.join(result)}\n\n🏆 Tu gagnes **{gain} jeton(s)** !\nSolde : **{data['joueurs'][gid][uid]['jetons']} jeton(s)**"
        couleur = 0x2ecc71
    else:
        titre = "🎰 Perdu !"
        desc = f"{'  '.join(result)}\n\nDommage... Réessaie !\nSolde : **{data['joueurs'][gid][uid]['jetons']} jeton(s)**"
...
async def loto_quotidien(interaction: discord.Interaction, tickets: int):
    ...
    solde = data.get("joueurs", {}).get(gid, {}).get(uid, {}).get("jetons", 0)
    if solde < tickets:
        await interaction.response.send_message(f"❌ Tu n'as que **{solde} jeton(s)**, il t'en faut **{tickets}**.", ephemeral=True)
        return
    data["joueurs"][gid][uid]["jetons"] -= tickets
