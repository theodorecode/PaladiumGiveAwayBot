...
# ══════════════════════════════════════════════
#  SYSTÈME D'INVITATIONS — WELCOME
# ══════════════════════════════════════════════

# Stocker les invitations avant qu'un membre arrive
invite_cache = {}
...
async def on_member_join(member):
    ...
    # Vérifier si fake compte (créé il y a moins de 5 minutes)
    account_age = datetime.now(timezone.utc) - member.created_at
    if account_age.total_seconds() < 300:
        is_fake = True
    ...
    if welcome_channel_id:
        ...
        if channel:
            ...
            if is_fake:
                embed.add_field(
                    name="⚠️ Compte suspect",
                    value="Ce compte a été créé **il y a moins de 5 minutes**. Possible fake account !",
                    inline=False
                )
...
# ══════════════════════════════════════════════
#  /setup-welcome — Configurer le système
# ══════════════════════════════════════════════
@bot.tree.command(name="setup-welcome", description="⚙️ Configurer le salon et rôle de bienvenue")
@app_commands.describe(
    salon="Salon où poster les messages de bienvenue",
    role="Rôle à donner automatiquement aux nouveaux membres"
)
...
async def setup_welcome(
    ...
    config[str(interaction.guild.id)] = {
        "welcome_channel": str(salon.id),
        "welcome_role": str(role.id)
    }
    save_config(config)

    embed = discord.Embed(
        title="✅ Système de bienvenue configuré",
        description=(
            f"**Salon :** {salon.mention}\n"
            f"**Rôle auto :** {role.mention}\n\n"
            f"Les nouveaux membres recevront automatiquement le rôle et un message de bienvenue avec l'info sur qui les a invités."
        ),
        color=0x2ecc71
    )
